...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx, search: str, *, loop, download=False):
        ...
        await ctx.send(f'```ini\n[Ajout de {data["title"]} dans la queue.]\n```')
...
class MusicPlayer(commands.Cog):
    ...
    async def player_loop(self):
        ...
        while not self.bot.is_closed():
            ...
            self.np = await self._channel.send(f'**Je joue :** `{source.title}` demander par '
                                               f'`{source.requester}`')
...
class Music(commands.Cog):
    ...
    async def __error(self, ctx, error):
        ...
        if isinstance(error, commands.NoPrivateMessage):
            try:
                return await ctx.send('Cette commande ne peut pas être utiliser en messages privés')
            except discord.HTTPException:
                pass
        elif isinstance(error, InvalidVoiceChannel):
            await ctx.send('Erreur de connection au channel vocal'
                           'Veuillez être sûr que vous êtes dans un channel vocal ou dans un channel que le bot peut accéder')
    ...
    async def connect_(self, ctx):
        try:
            ...
            raise InvalidVoiceChannel('Aucun channel à rejoindre')
        ...
        if vc:
            ...
            try:
                ...
                raise VoiceConnectionError(f'Changement de channel : <{channel}> impossible')
        else:
            try:
                await channel.connect()
            except asyncio.TimeoutError:
                raise VoiceConnectionError(f'Connexion au channel : <{channel}> impossible')
    ...
    async def pause_(self, ctx):
        ...
        if not vc or not vc.is_playing():
            return await ctx.send('Aucun musique en cours !')
        elif vc.is_paused():
            return

        vc.pause()
        await ctx.send(f'**`{ctx.author}`**: Musique mise en pause')
    ...
    async def resume_(self, ctx):
        ...
        if not vc or not vc.is_connected():
            return await ctx.send("Aucune musique n'avait été mise en pause")
        elif not vc.is_paused():
            return

        vc.resume()
        await ctx.send(f'**`{ctx.author}`**: Reprise du morceau')
    ...
    async def skip_(self, ctx):
        ...
        vc.stop()
        await ctx.send(f'**`{ctx.author}`**: Passage à la musique suivante')
    ...
    async def queue_info(self, ctx):
        ...
        if player.queue.empty():
            return await ctx.send("Il n'y a pas grand chose par ici")


        upcoming = list(itertools.islice(player.queue._queue, 0, 5))

        fmt = '\n'.join(f'**`{_["title"]}`**' for _ in upcoming)
        embed = discord.Embed(title=f'Musique à suivre {len(upcoming)}', description=fmt)
    ...
    async def now_playing_(self, ctx):
        ...
        if not player.current:
            return await ctx.send('Je ne joue actuellement aucun morceau')
        ...
        player.np = await ctx.send(f'**Je joue :** `{vc.source.title}` '
                                   f'Demander par `{vc.source.requester}`')
    ...
    async def change_volume(self, ctx, *, vol: float):
        ...
        if not 0 < vol < 101:
            return await ctx.send("Merci d'entrée une valeur comprise entre 1 et 100")
        ...
        await ctx.send(f'**`{ctx.author}`**: Changement du volume sur **{vol}%**')
