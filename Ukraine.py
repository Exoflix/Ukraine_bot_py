...
async def help(ctx):
    ...
    test_e.add_field(name="Annonces importantes :", value="***Ne cliquez pas sur les liens comme un site prétendant être un site de notre alliance si vous tenez à ne pas vous faire hack (IP Logger, etc.***")
    test_e.add_field(name="Bot", value="Version : 0.8_DYBot | Mise à jour le 10/10/21 à 12h01", inline=False)
    test_e.set_footer(text="YoRHa", icon_url=bot.user.avatar_url)
    test_e.set_author(name=f"{author}", icon_url=author.avatar_url)
...
async def maj(ctx):
    ...
    test_e.add_field(name="Mise à jour du 11/10/21 ", value="Ajout de la commande ***y!pays*** permettant de voir la liste officiel des pays de l'alliance", inline=False)
    await ctx.send(embed=test_e)
